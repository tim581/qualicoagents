...
## Key P&L Rules
- **3PL Storage** (Van Thiel, Forceget, WePrep, Monta): **OVERHEAD — below Contribution Margin**. NOT a per-unit-sold cost. Cost depends on inventory volume stored (semi-variable, seasonality curve). Annual total ~€19,415. `storage_3pl_eur` in margins = 0. Actual costs from `3PL_Warehousing_Costs` table. P&L line: "3PL Warehousing".
